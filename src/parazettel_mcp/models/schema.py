...
class LinkType(str, Enum):
    ...
    # Action-item structural links
    PART_OF = "part_of"  # This task/note belongs to a project or area
...
class NoteType(str, Enum):
    ...
    # Action-item types (PARA/GTD)
    TASK = "task"  # Atomic actionable item (GTD next action/waiting/someday)
...
class NoteStatus(str, Enum):
    ...
    INBOX = "inbox"  # Captured but not yet processed — needs triage
...
class NoteSource(str, Enum):
    """Origin of a note or action item."""

    MANUAL = "manual"  # Written directly
    INBOX = "inbox"  # Captured to inbox, unprocessed
    EMAIL = "email"
    MEETING = "meeting"
    VOICE = "voice"
...
class Note(BaseModel):
    ...
    due_date: Optional[datetime.date] = Field(
        default=None, description="Due date (task-type notes only)"
    )
    priority: Optional[int] = Field(
        default=None, description="Priority 1 (low) to 4 (critical)"
    )
    recurrence_rule: Optional[str] = Field(
        default=None,
        description="Recurrence interval: daily, weekly, monthly, quarterly, yearly",
    )
